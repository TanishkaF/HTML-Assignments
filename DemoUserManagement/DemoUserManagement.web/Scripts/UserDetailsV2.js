...
//function getCountry() {
//    $.ajax({
//        url: 'UserDetailsV2.aspx/GetCountry',
//        type: 'POST',
//        contentType: 'application/json',
//        dataType: 'json',
//        success: function (response) {
...
function populateSelectedCountryDropdown(countryID, prefix) {
    $.ajax({
        type: "POST",
        ...
        success: function (response) {
            if (response && response.d) {
                var selectedCountryName = response.d;
                $("#" + prefix + "Country").val(selectedCountryName); // Select the country in the dropdown
                console.log("Selected country:", selectedCountryName);
            } else {
                console.error("Empty or invalid response received while populating selected country.");
            }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            console.error("Error getting selected country:", thrownError);
        }
    });
}
...
function getCountry() {
    $.ajax({
        ...
        success: function (response) {
            ...
            ///var addressDetail = 
            var prefix = 'c'
            populateSelectedCountryDropdown(1, prefix);
        },
        error: function (xhr, status, error) {
            console.error("Error fetching countries:", error);
        }
